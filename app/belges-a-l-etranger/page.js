...
export default function BelgiansAbroadPage() {
  ...
  const content = {
    ...
    players: <div id="all-players" className="scroll-mt-24"><div className="mb-5 grid gap-2 rounded-2xl border border-line/10 bg-surface/70 p-3 sm:grid-cols-2 lg:grid-cols-4"><label className="flex items-center gap-2 rounded-xl border border-line/10 bg-bg/50 px-3"><Search className="h-4 w-4 text-muted" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Joueur ou club…" className="min-w-0 flex-1 bg-transparent py-2 text-sm outline-none" /></label><select value={country} onChange={(event) => setCountry(event.target.value)} className="rounded-xl border border-line/10 bg-bg/50 px-3 py-2 text-sm"><option value="all">Tous les pays</option>{countries.map((item) => <option key={item}>{item}</option>)}</select><select value={competition} onChange={(event) => setCompetition(event.target.value)} className="rounded-xl border border-line/10 bg-bg/50 px-3 py-2 text-sm"><option value="all">Tous les championnats</option>{competitionNames.map((item) => <option key={item}>{item}</option>)}</select><select value={position} onChange={(event) => setPosition(event.target.value)} className="rounded-xl border border-line/10 bg-bg/50 px-3 py-2 text-sm"><option value="all">Tous les postes</option>{["Gardien", "Défenseur", "Milieu", "Attaquant"].map((item) => <option key={item}>{item}</option>)}</select></div>{view.enriched.length === 0 ? <div className="rounded-3xl border border-dashed border-line/20 bg-surface/50 px-6 py-12 text-center"><div className="text-3xl">🇧🇪</div><h2 className="mt-3 text-xl font-black">Le suivi international est prêt</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted">Ajoutez une compétition étrangère dans l'admin, masquez-la du portail si nécessaire, puis activez « Suivi Belfoot » sur les joueurs retenus.</p></div> : shown.length === 0 ? <p className="rounded-2xl border border-line/10 bg-surface p-5 text-sm text-muted">Aucun joueur ne correspond à ces filtres.</p> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{shown.map(({ player, club, country: playerCountry, competition: playerCompetition, totals }) => <Link key={player.id} href={`/players/${player.id}`} className="group relative overflow-hidden rounded-2xl border border-line/10 bg-gradient-to-br from-surface to-bg/50 p-4 transition hover:-translate-y-0.5 hover:border-accent/40 hover:shadow-[0_20px_45px_-28px_rgba(227,6,19,0.65)]"><div className="flex items-start gap-3"><PlayerPhoto player={player} /><div className="min-w-0 flex-1"><h3 className="truncate text-lg font-black group-hover:text-accent">{player.name}</h3><div className="mt-1 flex items-center gap-2 text-xs text-muted">{club?.logo_url && <img src={club.logo_url} className="h-5 w-5 object-contain" alt="" />}<span className="truncate">{club?.name || "Club à renseigner"}</span></div><div className="mt-2 text-[11px] text-muted"><span className="mr-1">{FLAGS[playerCountry] || "🌍"}</span>{playerCountry} · {playerCompetition}</div></div></div><div className="mt-4 grid grid-cols-4 gap-1 border-t border-line/10 pt-3 text-center"><div><b className="block text-lg">{totals.appearances}</b><span className="text-[9px] uppercase tracking-wider text-muted">Matchs</span></div><div><b className="block text-lg">{totals.goals}</b><span className="text-[9px] uppercase tracking-wider text-muted">Buts</span></div><div><b className="block text-lg">{totals.assists}</b><span className="text-[9px] uppercase tracking-wider text-muted">Passes</span></div><div><b className="block text-lg">{totals.rating?.toFixed(1) || "—"}</b><span className="text-[9px] uppercase tracking-wider text-muted">Note</span></div></div></Link>)}</div>}</div>,
  };
  ...
  return (
    <div className="relative">
      ...
      {!data.loading && !data.error && <>
        ...
        {view.enriched.length > 0 && <div className="mt-9 space-y-9">{config.sections.filter((section) => section.enabled && section.key !== "players").map((section) => <section key={section.key}><SectionHeader section={section} />{content[section.key]}</section>)}</div>}
      </>}
    </div>
  );
}
