...
export async function syncSquads(db, competition, ctx = {}) {
  ...
  const season = (String(ctx.season ?? "").match(/\d{4}/) || [])[0] || null;
  ...
  for (const club of clubs || []) {
    ...
    for (const p of players) {
      ...
      if (pid && p.stats) {
        await db.from("player_season_stats").upsert({ player_id: pid, competition_id: competition.id, season, ...p.stats, source: competition.provider, external_id: p.external_id, synced_at: now }, { onConflict: "player_id,season" });
      }
      n++;
    }
  }
