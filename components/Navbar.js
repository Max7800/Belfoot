...
export default function Navbar() {
  ...
  useEffect(() => {
    const load = () => supabase.from("matches").select("*", { count: "exact", head: true }).eq("status", "live").then(({ count }) => setLiveCount(count || 0));
    load();
    const channel = supabase.channel("navbar-live-count").on("postgres_changes", { event: "*", schema: "public", table: "matches" }, load).subscribe();
    return () => { supabase.removeChannel(channel); };
  }, []);
  const label = (item) => <span className="inline-flex items-center gap-1.5">{item.label}{item.to === "/matchs" && liveCount > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-red-500/15 px-1.5 py-0.5 text-[9px] font-black text-red-400"><i className="h-1.5 w-1.5 animate-pulse rounded-full bg-red-500" />{liveCount}</span>}</span>;
  return (
    <header className="border-b border-line/10">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        ...
        <nav className="hidden min-w-0 items-center gap-3 whitespace-nowrap text-xs lg:flex lg:gap-4 lg:text-sm">
          {navItems().map((i) => <Link key={i.to} href={i.to} className="text-muted hover:text-content">{label(i)}</Link>)}
      ...
      {mobileOpen && <nav className="border-t border-line/10 bg-bg/95 px-4 py-3 backdrop-blur lg:hidden">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-2">
          {navItems().map((item) => <Link key={item.to} href={item.to} onClick={closeMobile} className="rounded-xl border border-line/10 bg-surface/60 px-3 py-2.5 text-sm font-semibold">{label(item)}</Link>)}
