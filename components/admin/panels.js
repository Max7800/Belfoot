...
export function HomePanel() {
  ...
  useEffect(() => { Promise.all([
    ...
    supabase.from("competitions").select("id,name,logo_url,public_visible,position").order("position", { ascending: true, nullsFirst: false }).order("name"),
  ]).then(([settingsResult, competitionResult]) => {
    setDraft(normalizeHomeConfig(settingsResult.data?.data?.home || {}));
  ...
  const setCompetitionCarousel = (key, value) => setDraft((current) => ({ ...current, competition_carousel: { ...current.competition_carousel, [key]: value } }));
  const toggleCarouselCompetition = (id) => {
    const selected = draft.competition_carousel?.competition_ids || [];
    setCompetitionCarousel("competition_ids", selected.includes(id) ? selected.filter((item) => item !== id) : [...selected, id]);
  };
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("site_settings").update({ data: { ...(data?.data || {}), home: { hero: draft.hero, competition_carousel: draft.competition_carousel, sections } } }).eq("id", 1);
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-xl border border-sky-400/20 bg-sky-400/5 p-4"><h3 className="font-bold">Carrousel des compétitions</h3><p className="mt-1 text-xs leading-5 text-muted">Choisis les compétitions accessibles depuis l'accueil et celle affichée en premier cette semaine. Cela ne change pas leur visibilité dans le portail Compétitions.</p><label className="mt-3 block text-xs text-muted">Compétition mise en avant<select value={draft.competition_carousel?.featured_competition_id || ""} onChange={(event) => setCompetitionCarousel("featured_competition_id", event.target.value)} className="mt-1 w-full rounded border border-line/10 bg-surface2 px-2 py-2 text-sm text-content"><option value="">Première compétition de la liste</option><option value="followed-belgians">Matchs des Belges suivis</option>{competitions.map((competition) => <option key={competition.id} value={competition.id}>{competition.name}{competition.public_visible === false ? " — masquée du portail" : ""}</option>)}</select></label><div className="mt-3 text-xs text-muted">Compétitions proposées dans le carrousel</div><p className="mt-1 text-[11px] text-muted">Si aucune case n'est cochée, toutes les compétitions publiques sont proposées.</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><label className="flex min-w-0 items-center gap-2 rounded-lg border border-amber-400/20 bg-amber-400/5 px-3 py-2 text-xs"><input type="checkbox" checked={(draft.competition_carousel?.competition_ids || []).includes("followed-belgians")} onChange={() => toggleCarouselCompetition("followed-belgians")} /><span className="min-w-0 flex-1 truncate">Matchs des Belges suivis</span><span className="shrink-0 text-[10px] text-amber-300">Vue spéciale</span></label>{competitions.map((competition) => <label key={competition.id} className="flex min-w-0 items-center gap-2 rounded-lg border border-line/10 bg-surface2 px-3 py-2 text-xs"><input type="checkbox" checked={(draft.competition_carousel?.competition_ids || []).includes(competition.id)} onChange={() => toggleCarouselCompetition(competition.id)} /><span className="min-w-0 flex-1 truncate">{competition.name}</span>{competition.public_visible === false && <span className="shrink-0 text-[10px] text-amber-300">Hors portail</span>}</label>)}</div></div>
...
export function NationalTeamsPanel() {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("site_settings").update({ data: { ...(data?.data || {}), national_teams: { hero: draft.hero, labels: draft.labels, sections } } }).eq("id", 1);
    setStatus(error ? "error" : "saved");
  };
  return <div>
    ...
    <div className="mb-6 rounded-xl border border-line/10 bg-surface p-4"><h3 className="mb-1 font-bold">Textes des modules</h3><p className="mb-3 text-xs text-muted">Les chiffres et les matchs sont automatiques ; tous les intitulés visibles restent éditables.</p><div className="grid gap-3 sm:grid-cols-2">
      {[["featured", "Bloc match principal"], ["all_matches", "Lien tous les matchs"], ["fifa", "Bloc classement FIFA"], ["ratings", "Bloc notation"], ["stats_matches", "Stat matchs"], ["stats_wins", "Stat victoires"], ["stats_goals", "Stat buts"], ["stats_fifa", "Stat classement FIFA"], ["show_all_results", "Déplier les résultats"], ["show_less_results", "Réduire les résultats"], ["show_all_players", "Déplier la sélection"], ["show_less_players", "Réduire la sélection"]].map(([key, label]) => <label key={key} className="text-xs text-muted">{label}<input value={draft.labels[key] || ""} onChange={(e) => setDraft((current) => ({ ...current, labels: { ...current.labels, [key]: e.target.value } }))} className="mt-1 w-full rounded border border-line/10 bg-surface2 px-2 py-2 text-sm text-content" /></label>)}
    </div></div>
