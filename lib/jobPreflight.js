...
async function competitionContext(db, ctx) {
  ...
  let query = db.from("matches").select("id,status,home_club_id,away_club_id,external_id").eq("competition_id", competition.id);
  if (season?.id) query = query.eq("season_id", season.id);
  const { data: matches, error: matchesError } = await query.limit(500);
...
async function estimateJob(db, key, ctx, context) {
  ...
  if (key === "football.events") {
    const eligible = context.matches.filter((match) => match.status === "finished" && match.external_id).length;
    return { min: 0, max: Math.min(cap, eligible), basis: `${eligible} match(s) terminé(s), plafond ${cap}` };
  }
  if (key === "football.lineups") {
    const eligible = context.matches.filter((match) => match.status === "finished" && match.external_id).length;
    const selected = Math.min(Math.min(cap, 20), eligible);
    return { min: 0, max: selected * 2, basis: `${selected} match(s), jusqu’à 2 requêtes par match` };
  }
  ...
  if (key === "football.live-sync") {
    ...
    for (const competition of competitions || []) {
      ...
      liveMatches += Math.min(cap, count || 0);
    }
