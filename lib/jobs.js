...
import footballPlayerCareers from "@/modules/football/jobs/playerCareers";
import { jobKeys } from "@/lib/jobCatalog";
import { preflightJobs } from "@/lib/jobPreflight";

const JOBS = { "football.sync": footballSync, "football.live-sync": footballLiveSync, "football.discover-belgians": footballDiscover, "football.track-belgians": footballTrack, "football.player-careers": footballPlayerCareers, "football.squads": footballSquads, "football.events": footballEvents, "football.coaches": footballCoaches, "football.lineups": footballLineups, "football.team-test": footballTeamTest, "football.national-team": footballNationalTeam, "football.find-national-teams": footballFindNationalTeams, "football.resolve-national-clubs": footballResolveNationalClubs };
...
export async function runJob(key, ctx = {}) {
  ...
  // Le préflight serveur bloque cible, migrations, quota connu et budget avant
  // tout appel provider ou création d'une exécution.
  const preflight = await preflightJobs(db, { key, ...ctx });
