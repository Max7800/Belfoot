...
export async function runJob(key, ctx = {}) {
  ...
  // Garde-fou migrations : si le job déclare une migration requise, on vérifie
  // qu'elle est appliquée AVANT de consommer le moindre appel API ou de tracer.
  const requiresMigration = JOB_CATALOG[key]?.requiresMigration;
  if (requiresMigration) {
    const { data: mig } = await db.from("schema_migrations").select("version").eq("version", requiresMigration).maybeSingle();
    if (!mig) throw new Error(`Migration requise non appliquée : « ${requiresMigration} ». Applique-la dans Supabase avant de lancer ce job.`);
  }
